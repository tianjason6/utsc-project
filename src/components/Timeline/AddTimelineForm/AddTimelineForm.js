...
class AddTimelineForm extends Component {
    ...
    renderField = ({ input, meta, label, type, name, accept, className }) => (
        <div className={
            ...
            {type!='file'
                ? ( label === 'Content' ? 
                    <textarea className={className} {...input} name={name} type={type} rows='1'>testing</textarea>
                    :
    ...
    fileUploadHandler = (id) => {
        ...
        const binaryData = [this.state.fileBlob]
    ...
    submitForm = (values) => {
        ...
        const postDate = '' + currentTime.getFullYear() + '/' + (currentTime.getMonth() + 1) + '/'+currentTime.getDate();
        let postTime = '';
        let amPm = 'pm';
        if(currentTime.getHours() < 13 ) {
            currentTime.getHours() === 0 ? postTime += '12' : postTime += currentTime.getHours();
            amPm = 'am';
        } else {
        ...
        (''+currentTime.getMinutes()).length === 1 ? (postTime = postTime + ':0' + currentTime.getMinutes()) : (postTime += ':' + currentTime.getMinutes());
        postTime += amPm;
    ...
    render(){
        ...
        return(
            <form className={styles.background} onSubmit={handleSubmit(this.submitForm)} >
                {this.config()}
                <h3 className={styles.banner}>Post a new timeline item! (Admin Feature)</h3>
                ...
                />
                <button className={styles.button} type='submit' >Post to Timeline</button>
            </form>
        )
    }
...
const mapDispatchToProps = (dispatch) => {
    return {
        addToTimeline: (date, time, username, content, id, file) => dispatch(timelineAction.addToTimeline(date, time, username, content, id, file))
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(AddTimelineForm);