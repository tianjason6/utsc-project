...
import styles from "./FeaturedModal.module.css";
  ...
  render() {
    return (
      <div className={styles.confirmBox}>
        <p>
          Delete {this.props.modalProps.projectTitle} from Featured Projects?
        </p>
        <button
          onClick={() => {
            this.removeFeatured();
...
const mapDispatchToProps = dispatch => {
  return {
    changeArchiveStatus: (status, pTitle) =>
      dispatch(archiveStatusActions.changeArchiveStatus(status, pTitle)),
    removeFeaturedProject: newFeaturedList =>
      dispatch(FeaturedProjectActions.removeFeaturedProject(newFeaturedList)),
